...
public abstract class AbstractCard implements Comparable<AbstractCard> {
    ...
    private String getDynamicValue(char key) {
        switch (key) {
            ...
            default:
                logger.info("KEY: " + key, new Object[0]);
    ...
    public static int getPrice(CardRarity rarity) {
        switch (rarity) {
            case BASIC:
                logger.info("ERROR: WHY WE SELLIN' BASIC", new Object[0]);
                return 9999;
            case CURSE:
            default:
                logger.info("No rarity on this card?", new Object[0]);
                return 0;
            case SPECIAL:
                logger.info("ERROR: WHY WE SELLIN' SPECIAL", new Object[0]);
                return 9999;
            case COMMON:
                return 50;
            case UNCOMMON:
    ...
    public void updateCost(int amt) {
        if (this.cost < -1) {
            logger.info("Unplayable cards cannot have their costs modified", new Object[0]);
        } else {
    ...
    public boolean freeToPlay() {
        if (this.freeToPlayOnce) {
            ...
            return AbstractDungeon.player != null && AbstractDungeon.currMapNode != null && AbstractDungeon.getCurrRoom().phase == RoomPhase.COMBAT && AbstractDungeon.player.hasPower("FreeAttackPower") && this.type == CardType.ATTACK;
        }
    }
    ...
    public void applyPowers() {
        ...
        // 检查是否带有SCIENCE标签
        if (this.hasTag(CardTags.SCIENCE)) {
            // 获取specialEnergy能力层数
            AbstractPower power = player.getPower("LexKela");
            if (power != null && power.amount >= 0 ) {
                // 动态修改费用（显示用）
                this.costForTurn = Math.max(0, this.cost - power.amount);
                this.isCostModifiedForTurn = true;
            }
            else {
                this.costForTurn = this.cost;
    ...
    public static void debugPrintDetailedCardDataHeader() {
        logger.info(gameDataUploadHeader(), new Object[0]);
    }
    ...
    public void debugPrintDetailedCardData() {
        logger.info(this.gameDataUploadData(), new Object[0]);
    }
