...
import sgsMod.cards.Others.BaoLi;
...
public class baoli extends AbstractPower {
    public static final String POWER_ID = baoli.class.getSimpleName();
    ...
    public baoli(AbstractCreature owner) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = -1;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/baoli.png");
        this.ctr = false;
    ...
    public void atStartOfTurn() {
        ...
        this.flash();
        this.addToBot(new SFXAction("XiongHuo"));
        int res = AbstractDungeon.cardRandomRng.random(0, 2);
        if(res == 0){
            this.addToBot(new ApplyPowerAction(this.owner, p, new WeakPower(this.owner, 1 , false)));
            this.addToBot(new FireDamageAction(this.owner, new DamageInfo(p, 10, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.SLASH_HORIZONTAL));
            ...
            this.addToBot(new ApplyPowerAction(this.owner, this.owner, new StrengthPower(this.owner, -2)));
    ...
    public void onDeath() {
        if (!AbstractDungeon.getMonsters().areMonstersBasicallyDead() && this.owner.currentHealth <= 0) {
            this.addToBot(new SFXAction("ShaJue"));
            this.addToBot(new MakeTempCardInHandAction(new BaoLi()));
