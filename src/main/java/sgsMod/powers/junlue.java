...
public class junlue extends AbstractPower {
    public static final String POWER_ID = junlue.class.getSimpleName();
    ...
    public junlue(AbstractCreature owner) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = -1;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/junlue.png");
    ...
    public void onAttack(DamageInfo info, int damageAmount, AbstractCreature target) {
        if (damageAmount > 0 && target != this.owner && info.type == DamageInfo.DamageType.NORMAL) {
            if(ctrl){
                this.addToBot(new LineAction("JunLue", SoundController.SoundType.Battle));
                ctrl = false;
            }
            this.addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new junluebiaoji(AbstractDungeon.player, 1)));
        }
    }
