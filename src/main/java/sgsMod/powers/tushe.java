...
import sgsMod.actions.unique.TuSheAction;
...
public class tushe extends AbstractPower {
    public static final String POWER_ID = tushe.class.getSimpleName();
    ...
    public tushe(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/tushe.png");
    ...
    public void onAfterCardPlayed(final AbstractCard usedCard) {
        this.addToBot(new TuSheAction(AbstractDungeon.player, this.amount));
