...
public class jili extends AbstractPower {
    public static final String POWER_ID = jili.class.getSimpleName();
    ...
    public jili(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/jili.png");
        this.ctr = true;
    ...
    public void onAfterUseCard(AbstractCard card, UseCardAction action) {
        ...
        if(card.costForTurn != 0 && !card.freeToPlayOnce){
            if(card.costForTurn == AbstractDungeon.actionManager.cardsPlayedThisTurn.size()){
                this.addToBot(new LineAction("JiLi", SoundController.SoundType.Turn));
                this.addToBot(new GainEnergyAction(this.amount * card.costForTurn));
            }
        }
    }
