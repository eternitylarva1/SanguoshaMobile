...
public class quanji extends AbstractPower {
    public static final String POWER_ID = quanji.class.getSimpleName();
    ...
    public quanji(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/quanji.png");
        this.ctr = false;
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (damageAmount > 0 && info.type == DamageInfo.DamageType.NORMAL) {
            if(ctr){
                this.addToBot(new SFXAction("QuanJi"));
                ctr = false;
            }
            this.addToBot(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new quan(AbstractDungeon.player, this.amount)));
        }
        return damageAmount;
