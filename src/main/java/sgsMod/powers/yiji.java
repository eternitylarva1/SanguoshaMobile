...
public class yiji extends AbstractPower {
    public static final String POWER_ID = yiji.class.getSimpleName();
    ...
    public yiji(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/yiji.png");
    ...
    public void wasHPLost(DamageInfo info, int damageAmount) {
        if (damageAmount > 0  && info.type != DamageInfo.DamageType.HP_LOSS) {
            ...
            this.addToBot(new SFXAction("YiJi"));
