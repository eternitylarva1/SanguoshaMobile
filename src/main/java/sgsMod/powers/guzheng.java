...
import sgsMod.actions.unique.GuZhengAction;
...
public class guzheng extends AbstractPower {
    public static final String POWER_ID = guzheng.class.getSimpleName();
    ...
    public guzheng(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/guzheng.png");
        this.block = 0;
    ...
    public void updateDescription() {
        int d = GameActionManager.totalDiscardedThisTurn - 1;
        if(d < 0) d = 0;
        block = d * this.amount;
        if(DESCRIPTIONS.length == 1){
            ...
            this.description = (DESCRIPTIONS[0] + amount + DESCRIPTIONS[1] + block + DESCRIPTIONS[2]);
        }
    }
    ...
    public void atStartOfTurn() {
        this.block = 0;
        this.addToBot(new GuZhengAction(this));
    }//触发时机：当玩家回合结束时触发。
    ...
    public void atEndOfTurn(final boolean isPlayer) {
        ...
        if(GameActionManager.totalDiscardedThisTurn > 1){
            this.addToBot(new SFXAction("GuZheng"));
            this.addToBot(new GainBlockAction(p, (GameActionManager.totalDiscardedThisTurn-1) * this.amount ));
        }
