...
public class buqu extends AbstractPower {
    public static final String POWER_ID = buqu.class.getSimpleName();
    ...
    public buqu(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.total = this.amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/buqu.png");
    ...
    public void updateDescription() {
        if(DESCRIPTIONS.length == 1){
            ...
            this.description = (DESCRIPTIONS[0] + amount + DESCRIPTIONS[1] + total + DESCRIPTIONS[2]);
        }
    }
    ...
    public int onLoseHp(final int damageAmount) {
        ...
        this.addToBot(new LineAction("BuQu"));
        int newAmount = damageAmount;
        if(newAmount >= this.amount){
            newAmount -= this.amount;
            this.addToBot(new RemoveSpecificPowerAction(p, p, this));
            this.addToBot(new DamageAction(p, new DamageInfo(p, this.total, DamageInfo.DamageType.THORNS)));
        }
        else{
