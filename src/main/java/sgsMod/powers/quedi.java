...
public class quedi extends AbstractPower {
    public static final String POWER_ID = quedi.class.getSimpleName();
    ...
    public quedi(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/quedi.png");
        this.ctr = false;
    ...
    public void onAfterUseCard(final AbstractCard card, final UseCardAction action) {
        if(card.cardID.equals(JueDou.ID) || card.hasTag(AbstractCard.CardTags.Sha)){
            this.addToBot(new SFXAction("QueDi"));
