...
public class yaowu extends AbstractPower {
    public static final String POWER_ID = yaowu.class.getSimpleName();
    ...
    public yaowu(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/yaowu.png");
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.type != DamageInfo.DamageType.THORNS && info.type != DamageInfo.DamageType.HP_LOSS && info.owner != null && info.owner != this.owner) {
            //this.flash();
            this.addToTop(new GainBlockAction(info.owner, this.amount));
            if(ctr){
                ctr = false;
                this.addToTop(new SFXAction("YaoWu"));
            }
        }
