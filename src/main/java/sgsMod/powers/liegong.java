...
public class liegong extends AbstractPower {
    public static final String POWER_ID = liegong.class.getSimpleName();
    public static final String NAME;
    public static final String[] DESCRIPTIONS;
    public static PowerStrings powerStrings;
    private boolean ctr;
    private final Map<AbstractCard.CardType, Boolean> m;
    ...
    public liegong(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/liegong.png");
        this.ctr = true;
        this.m = new HashMap<>();
    ...
    public void updateDescription() {
        if(DESCRIPTIONS.length == 1){
            ...
            this.description += "(已记录： ";
            if(m.containsKey(AbstractCard.CardType.ATTACK)){
                this.description += "攻击，";
            }
            if(m.containsKey(AbstractCard.CardType.SKILL)){
                this.description += "技能，";
            }
            if(m.containsKey(AbstractCard.CardType.POWER)){
                this.description += "能力，";
            }
            if(m.containsKey(AbstractCard.CardType.STATUS)){
                this.description += "状态，";
            }
            if(m.containsKey(AbstractCard.CardType.CURSE)){
                this.description += "诅咒，";
            }
            this.description += ")";
        }
    ...
    public void onUseCard(final AbstractCard card, final UseCardAction action) {
        ...
        if(card.hasTag(AbstractCard.CardTags.Sha)){
            ...
            if(!m.containsKey(card.type)){
                m.put(card.type, true);
                this.addToBot(new ApplyPowerAction(p, p, new liegongbiaoji(p, this.amount)));
            }
        }
