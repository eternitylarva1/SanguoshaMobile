...
public class ying extends AbstractPower {
    public static final String POWER_ID = ying.class.getSimpleName();
    ...
    public ying(AbstractCreature owner) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = -1;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/ying.png");
    ...
    public void atEndOfRound() {
        this.addToBot(new SFXAction("JieYing"));
        AbstractPlayer p = AbstractDungeon.player;
        if(!this.owner.hasPower(ArtifactPower.POWER_ID)){
            //有人工劫不到力量
            this.addToBot(new ApplyPowerAction(p, p, new StrengthPower(this.owner, 3)));
        }
        this.addToBot(new DrawCardAction(1));
        //this.addToBot(new ApplyPowerAction(p, p, new EnergizedPower(p, 2)));
        this.addToBot(new ApplyPowerAction(this.owner, this.owner, new StrengthPower(this.owner, -3)));
        this.addToBot(new RemoveSpecificPowerAction(this.owner, this.owner, this));
    }//触发时机：当怪物回合结束时触发。
