...
public class liegongbiaoji extends AbstractPower {
    public static final String POWER_ID = liegongbiaoji.class.getSimpleName();
    ...
    public liegongbiaoji(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/liegongbiaoji.png");
        this.ctr = true;
    ...
    public void onUseCard(final AbstractCard card, final UseCardAction action) {
        ...
        if(card.hasTag(AbstractCard.CardTags.Sha)){
            ...
            if(this.amount > 0){
                if(ctr){
                    this.addToBot(new LineAction("LieGong1"));
                }
                else{
                    this.addToBot(new LineAction("LieGong2"));
                }
                ctr = (!ctr);
            }
