...
public class juejin extends AbstractPower {
    public static final String POWER_ID = juejin.class.getSimpleName();
    ...
    public juejin(AbstractCreature owner) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = -1;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/juejin.png");
    ...
    public void onVictory() {
        ...
        if (p.currentHealth > 0 && p.currentBlock > 0) {
            p.heal(p.currentBlock);
        }
