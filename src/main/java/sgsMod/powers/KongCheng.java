...
public class KongCheng extends AbstractPower {
    public static final String POWER_ID = KongCheng.class.getSimpleName();
    ...
    public KongCheng(AbstractCreature owner) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = -1;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/kongcheng.png");
    ...
    public int onAttackedToChangeDamage(DamageInfo info, int damageAmount) {
        if(AbstractDungeon.player.hand.size() == 0 && info.type == DamageInfo.DamageType.NORMAL){
            this.flash();
