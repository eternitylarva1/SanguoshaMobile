...
public class jianying extends AbstractPower {
    public static final String POWER_ID = jianying.class.getSimpleName();
    ...
    public jianying(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/jianying.png");
        this.ctr = true;
    ...
    public void onUseCard(final AbstractCard card, final UseCardAction action) {
        if(AbstractDungeon.actionManager.cardsPlayedThisTurn.size() > 1){
            if(AbstractDungeon.actionManager.cardsPlayedThisTurn.get(AbstractDungeon.actionManager.cardsPlayedThisTurn.size() - 2).type == card.type ){
                String s;
                if(ctr){
                    s = "JianYing1";
                }
                ...
                this.addToBot(new LineAction(s, SoundController.SoundType.Rate));
                ctr = (!ctr);
