...
public class pojun extends AbstractPower {
    public static final String POWER_ID = pojun.class.getSimpleName();
    ...
    public pojun(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/pojun.png");
        this.ctr = false;
    ...
    public void onUseCard(final AbstractCard card, final UseCardAction action) {
        if(card.hasTag(AbstractCard.CardTags.Sha)){
            if(ctr){
                this.addToTop(new LineAction("PoJun1"));
            }
            else{
                this.addToTop(new LineAction("PoJun2"));
            }
            ctr = !ctr;
        }
