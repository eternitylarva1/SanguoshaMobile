...
public class yingjian extends AbstractPower {
    public static final String POWER_ID = yingjian.class.getSimpleName();
    ...
    public yingjian(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/yingjian.png");
    ...
    public void atStartOfTurn() {
        this.addToBot(new SFXAction("YingJian"));
        this.addToBot(new MakeVirtualCardInHandAction(new Sha(), this.amount));
        AbstractPlayer p = AbstractDungeon.player;
        this.addToBot(new ApplyPowerAction(p, p, new ShaTimesPower(p, this.amount)));
    }
