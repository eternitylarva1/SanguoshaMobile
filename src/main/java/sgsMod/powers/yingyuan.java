...
import sgsMod.actions.unique.YingYuanAction;
...
public class yingyuan extends AbstractPower {
    public static final String POWER_ID = yingyuan.class.getSimpleName();
    ...
    public yingyuan(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img =  AssetLoader.getTexture(SgsMod.MOD_ID,"img/power_img/yingyuan.png");
        this.ctr = true;
    ...
    public void onUseCard(final AbstractCard card, final UseCardAction action) {
        if(!card.hasTag(AbstractCard.CardTags.Temporary) && (card.hasTag(AbstractCard.CardTags.Basic) || card.hasTag(AbstractCard.CardTags.JinNang))){
            boolean used = false;
            for(int i = 0; i < AbstractDungeon.actionManager.cardsPlayedThisTurn.size() - 1; i++){
                if(AbstractDungeon.actionManager.cardsPlayedThisTurn.get(i).cardID.equals(card.cardID)){
                    used = true;
                    break;
                }
            }
            if(!used){
                this.addToBot(new LimitedLineAction("YingYuan", false));
                this.addToBot(new YingYuanAction(card, this.amount, true, true));
            }
        }
