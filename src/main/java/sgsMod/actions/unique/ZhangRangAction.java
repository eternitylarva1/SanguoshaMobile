...
public class ZhangRangAction extends AbstractGameAction {
    ...
    private final AbstractCard taoluan;

    public ZhangRangAction(AbstractCard.CardType t, AbstractCard taoluan) {
    ...
    public void update() {
        ...
        if (this.duration == Settings.ACTION_DUR_FAST) {
            ...
            if (this.cannotDuplicate.size() == this.p.hand.group.size()) {
                //没有指定手牌
                this.addToTop(new LoseHPAction(p, p, 10));
                this.isDone = true;
                return;
            }
        ...
        this.tickDuration();
        this.addToBot(new CardBackToHandAction(p, taoluan));
    }
