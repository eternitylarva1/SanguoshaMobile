...
import androidTestMod.actions.unique.CardBackToHandAction;
import androidTestMod.actions.unique.LineAction;
import androidTestMod.cards.JinNang.NanManRuQin;
...
public class juxiang extends AbstractPower {
    public static final String POWER_ID = juxiang.class.getSimpleName();
    ...
    public juxiang(AbstractCreature owner) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = -1;
        this.img = ImageMaster.loadImage("img/power_img/juxiang.png");
    ...
    public void onAfterUseCard(final AbstractCard card, final UseCardAction action) {
        if(card.cardID.equals(NanManRuQin.ID)){
            this.addToBot(new LineAction("JuXiang", SoundController.SoundType.Battle));
            this.addToBot(new CardBackToHandAction(AbstractDungeon.player, card));
        }
