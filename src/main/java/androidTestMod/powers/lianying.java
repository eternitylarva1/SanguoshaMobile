...
import androidTestMod.actions.unique.LianYingAction;
...
public class lianying extends AbstractPower {
    public static final String POWER_ID = lianying.class.getSimpleName();
    ...
    public lianying(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
        this.name = NAME;
        this.ID = POWER_ID;
        this.owner = owner;
        this.amount = amount;
        this.img = ImageMaster.loadImage("img/power_img/lianying.png");
    ...
    public void onAfterCardPlayed(final AbstractCard usedCard) {
        this.addToBot(new LianYingAction(AbstractDungeon.player, this.amount));
