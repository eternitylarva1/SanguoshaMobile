...
import androidTestMod.cards.Basic.Tao;
import androidTestMod.cards.JinNang.JueDou;
import androidTestMod.cards.JinNang.TaoYuanJieYi;
...
public class TaoLuanAction extends AbstractGameAction {
    ...
    private ArrayList<AbstractCard> generateCardChoices(boolean upgraded) {
        ...
        for(AbstractCard c2 : actionManager.cardsPlayedThisCombat){
            //如果本场战斗已经使用过某张牌，将它移除
            totalList.removeIf(c3 -> c2.cardID.equals(c3.cardID));
        }

        totalList.removeIf(c2 -> c2.cardID.equals(Shan.ID) || c2.cardID.equals(Tao.ID) || c2.cardID.equals(TaoYuanJieYi.ID));
